...
### Published dashboard
- `sync_dashboard.py` — builds the verified dashboard snapshot and binds the
  current ledgers into the static page
- `index.html` — GitHub Pages dashboard; paper-only and fail-closed when its
  verified snapshot is unavailable

## Canonical artifact families

These are generated by the pipeline. Source-heavy and rebuildable binary
intermediates should **not** be committed. The curated JSON/CSV publication
surface named in `artifact_integrity.py` is intentionally tracked so every
published dashboard state remains reviewable and reproducible:
