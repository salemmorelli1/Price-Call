...
PART3_SCRIPT = ROOT / "part3_governance.py"

PRED_LOG_PATH = ROOT / "artifacts_part3" / "prediction_log.csv"
PART3_TAPE_PATH = ROOT / "v141_final_production_tape.csv"
PART3_GOV_PATH = ROOT / "v141_final_production_governance.csv"
...
def print_path_status() -> None:
    ...
    print(f"PART3:     {PART3_SCRIPT} | exists={PART3_SCRIPT.exists()}")
    print(f"PRED_LOG:  {PRED_LOG_PATH} | exists={PRED_LOG_PATH.exists()}")
    print(f"TAPE:      {PART3_TAPE_PATH} | exists={PART3_TAPE_PATH.exists()}")
    print(f"GOV:       {PART3_GOV_PATH} | exists={PART3_GOV_PATH.exists()}")
...
def validate_outputs() -> None:
    if not PART3_TAPE_PATH.exists():
        raise FileNotFoundError(f"Missing Part 3 tape: {PART3_TAPE_PATH}")
    if not PART3_GOV_PATH.exists():
        raise FileNotFoundError(f"Missing Part 3 governance file: {PART3_GOV_PATH}")
    if not PRED_LOG_PATH.exists():
    ...
    if "Date" not in tape.columns:
        raise RuntimeError("v141_final_production_tape.csv is missing 'Date'.")

    tape["Date"] = pd.to_datetime(tape["Date"], errors="coerce").dt.normalize()
    tape = tape.dropna(subset=["Date"])
    if tape.empty:
        raise RuntimeError("v141_final_production_tape.csv has no valid Date rows.")
    ...
    print(f"production_tape: {PART3_TAPE_PATH}")
    print(f"governance_tape: {PART3_GOV_PATH}")
    print(f"validated decision_date: {expected_date.date()}")



def required_scripts_exist() -> bool:
    return PART1_SCRIPT.exists() and PART2_SCRIPT.exists() and PART3_SCRIPT.exists()
...
def main(force: bool = False, dry_run: bool = False, show_paths: bool = True) -> int:
    ...
    if not required_scripts_exist():
        missing = []
        if not PART1_SCRIPT.exists():
            missing.append(str(PART1_SCRIPT))
        if not PART2_SCRIPT.exists():
            missing.append(str(PART2_SCRIPT))
        if not PART3_SCRIPT.exists():
            missing.append(str(PART3_SCRIPT))
